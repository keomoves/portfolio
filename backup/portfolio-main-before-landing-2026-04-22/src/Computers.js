...
export function Computers({ interactionsDisabledProjects = false, ...props }) {
  ...
  return (
    <group {...props} dispose={null}>
      ...
      <ScreenInteractive
        ...
        onActivate={() =>
          window.dispatchEvent(
            new CustomEvent('focusShowreel', { detail: { target: [0.5, 0, -4] } })
          )
        }
      />
      <ScreenInteractive
        ...
        label="About Me"
        boxColor="#7a3cff"
        boxHoverColor="#9b6cff"
        textPosition={[-0.1, 3.5, 0]}
        fontSize={0.4}
        boxPosition={[-0.1, 4.5, -3]}
        boxScale={0.7}
        onActivate={() => window.dispatchEvent(new CustomEvent('focusCharacter', { detail: true }))}
      />
      ...
      <ScreenInteractive
        ...
        label="Projects"
        boxColor="royalblue"
        boxHoverColor="#3aa3ff"
        textPosition={[0, 0.7, 0]}
        fontSize={0.5}
        boxPosition={[0, 0.8, -3]}
        boxScale={0.7}
        disabled={interactionsDisabledProjects}
        onActivate={() =>
          window.dispatchEvent(
            new CustomEvent('focusProjects', { detail: { target: [0.63, 1.43, -5] } })
          )
        }
      />
...
function Screen({ frame, panel, children, ...props }) {
  ...
  const { onPointerOver, onPointerOut, onClick, ...rest } = props
  return (
    <group {...rest}>
...
/* Interactive text screen that triggers camera zoom */
function ScreenZoomText({ label = 'About Me', ...props }) {
  const onClick = (e) => {
    e.stopPropagation()
    window.dispatchEvent(new CustomEvent('focusCharacter', { detail: true }))
  }
  return (
    <Screen {...props} onClick={onClick}>
...
function ScreenInteractive({
  label = 'Showreel',
  boxColor = 'indianred',
  boxHoverColor = 'hotpink',
  textPosition = [-3.15, 0.75, 0],
  fontSize = 0.4,
  boxPosition = [-4, 1, -3],
  boxScale = 0.7,
  onActivate,
  ...
  const { openVideo, showVideo } = useContext(VideoModalContext)
  ...
  const disabled = !!showVideo || disabledProp
  return (
    <Screen
      {...props}
      onPointerOver={disabled ? undefined : handleScreenPointerOver}
      onPointerOut={disabled ? undefined : handleScreenPointerOut}
      onClick={disabled ? undefined : handleScreenClick}
    >
      <PerspectiveCamera makeDefault manual aspect={1 / 1} position={[0, 0, 10]} />
      ...
      <SpinningBox
        ...
        scale={boxScale}
        globalHover={disabled ? false : isScreenHovered}
        color={boxColor}
        hoverColor={boxHoverColor}
        videoUrl={onActivate ? undefined : 'https://www.youtube.com/embed/lw3WAqcI8YI?autoplay=1'}
      />
